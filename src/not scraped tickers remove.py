...
def get_correct_tickers():
    corrupted_df = pd.read_excel('D://selenium project/selenium/Stocks Data/9-India/Fiscal Year 2022 Update/Full Data FY2022 Update.xlsx')
    temp = list(corrupted_df['Ticker Symbol'])
...
def remove_ticker(tickers_list):
    ...
    with open('D://selenium project/selenium/Stocks Data/9-India/Fiscal Year 2022 Update/scraped ids.txt') as scraped_file:
        ...
        for id in scraped_list:
            if id.split(':')[0] in tickers_list: # for China: int() for others: nothing
...
correct_tickers = get_correct_tickers()

# Run Program
print(len(correct_tickers))
remove_ticker(correct_tickers)
